...
export const UpdateBookingStatus = async (id: number, newStatus: number): Promise<BookingDTO | null> => {
  try {
...
export const getBookingById = async (id: number): Promise<BookingDTO | null> => {
  try {
...
export const getBookingByUserId = async (): Promise<BookingDTO[]> => {
  try {
    const response = await api.get<BookingDTO[]>('/Booking/bookings/BookingByUserId');
    return response.data;
  } catch (error) {
...
export const deleteBooking = async (id: number): Promise<boolean> => {
  try {
    // Matches your controller action: [HttpDelete("bookings/{id:int}")]
